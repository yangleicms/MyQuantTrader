...
def check_response(json_wrapper):
    if json_wrapper.contain_key("success"):
        ...
        if success is False:
            ...
            if err_code == "":
                raise DeribitApiException(DeribitApiException.EXEC_ERROR, "[Executing] " + err_msg)
            else:
                raise DeribitApiException(DeribitApiException.EXEC_ERROR, "[Executing] " + str(err_code) + ": " + err_msg)
    elif json_wrapper.contain_key("code"):
        code = json_wrapper.get_int("code")
        msg = json_wrapper.get_string_or_default("msg", "")
        if code != 200:
            raise DeribitApiException(DeribitApiException.EXEC_ERROR, "[Executing] " + str(code) + ": " + msg)
